...
def qwk(y_pred, y_true):
    return quadratic_weighted_kappa(y_pred, y_true, min(y_true), max(y_true))

def eval_metrics(y_true, y_pred):
    accuracy = accuracy_score(y_true=y_true, y_pred=y_pred)
    f1_macro = f1_score(y_true=y_true, y_pred=y_pred, average='macro')
    # print({"accuracy": accuracy, "f1_macro": f1_macro})
    try:
        qwk_score = qwk(y_pred, y_true)
        return {"accuracy": accuracy, "f1_macro": f1_macro, "qwk": qwk_score}
    except:
